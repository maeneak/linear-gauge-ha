  ...
  `}(this._config.warnings??[],n,this._config.display??{})}
      ...
      `)}}if(r&&t.values.length>0){const e=t.values.filter(t=>Number.isFinite(t.value)&&0!==t.value);if(e.length>0){const t=At(e.reduce((t,e)=>t+e.value,0)/e.length,i);if("horizontal"===i.orientation){const e=i.trackY-2,o=3;a.push(V`
        ...
        `)}}return V`<g class="history-markers">${a}</g>`}(this._historyData,this._config,n)}
              ${function(t,e){const i={...mt,...t.ticks?.major};if(i.interval<=0)return V``;const s=[],{min:a,max:o}=e;for(let t=a;t<=o+1e-4;t+=i.interval){const a=At(t,e);if("horizontal"===e.orientation){const o=e.trackY+e.trackHeight+2,n=o+i.size;if(s.push(V`
